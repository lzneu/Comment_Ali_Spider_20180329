...
def get_Tmall_comment(commentID_list, requestDict, comments_collection, product_index, start_num, maxPage_num_control,
...
def get_one_page_comment(commentID_list, item_id, shop_id, title, catagory, page_num, comments_collection, product_index,
    ...
    if len(temp) > 1:
        ...
        for comment_info in rate_list:
            ...
            comment_info['name'] = title
            comment_info['catagory'] = catagory
            ...
            # comment_info['store_type'] = store_type
...
def getDataDataFrameFromMongo(db, collection, query):
    frame = read_mongo(db, collection, query=query)
...
def main(log_collection_name, product_index, page_nummber, catagory_collection_name, target_collexction_name,
    ...
    if (not Test):
        ...
        # catagory_collection_name =sys.argv[4]
        ...
        catagory_collection_name = catagory_collection_name
    ...
    df = getDataDataFrameFromMongo(db, catagory_collection_name, {})
    ...
    catagory_list = df['catagory'].values
    ...
    try:
        ...
        target_dict['catagory'] = str(catagory_list[product_index])
    ...
    get_Tmall_comment(commentID_list, target_dict, comments_collection, product_index, startNum, maxPage_num_control,
    ...
    for i in range(product_index + 1, len(df)):  # 73   573     #1073    1577
        ...
        target_dict['catagory'] = str(catagory_list[i])
        ...
        commentID_list = get_commentID_list(target_dict['original_id'], indexCollection)
        get_Tmall_comment(commentID_list, target_dict, comments_collection, i, startNum, maxPage_num_control, log_collection,
                          indexCollection)
