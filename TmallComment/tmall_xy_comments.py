...
from toollib.logger_tmall_xy import Logger
...
def get_Tmall_comment(product_dic, commentID_list, requestDict, comments_collection, product_index, start_num, maxPage_num_control,
...
def get_one_page_comment(product_dic, commentID_list, item_id, shop_id, title, category, page_num, comments_collection, product_index,
    ...
    if len(temp) > 1:
        ...
        for comment_info in rate_list:
            ...
            comment_info['shop_type'] = int(product_dic['shop_type'])
...
def insertToMongo(commentID_list, comments_collection, oneDict, indexCollection):
    if oneDict['comment_id'] not in commentID_list:
        ...
        comments_collection.insert(oneDict)
...
def main(log_collection_name, product_index, page_nummber, category_collection_name, target_collexction_name,
    ...
    df = getDataDataFrameFromMysql(isLocal, category_collection_name)
    ...
    df['shop_type'] = df['shop_type'].astype('int')
    ...
    brand_id_list = df['brand_id'].values
    shop_type_list = (df['shop_type'].values)
    category_id_list = df['category_id'].values
    ...
    try:
        ...
        target_dict['brand_id'] = str(brand_id_list[product_index])
        target_dict['shop_type'] = shop_type_list[product_index]
        target_dict['category_id'] = str(category_id_list[product_index])
    ...
    get_Tmall_comment(target_dict, commentID_list, target_dict, comments_collection, product_index, startNum, maxPage_num_control,
    ...
    for i in range(product_index + 1, len(df)):  # 73   573     #1073    1577
        ...
        target_dict['brand_id'] = str(brand_id_list[product_index])
        target_dict['shop_type'] =int(shop_type_list[product_index])
        target_dict['category_id'] = str(category_id_list[product_index])

        startNum = 1

        commentID_list = get_commentID_list(target_dict['original_id'], indexCollection)
        get_Tmall_comment(target_dict, commentID_list, target_dict, comments_collection, i, startNum, maxPage_num_control, log_collection,
                          indexCollection)
