    ...
    if isLocal:
       ...
       WHERE (CATEGORY_TYPE = '%d' AND CATEGORY_ID = '%d' AND STORE_ID = '%d')" % (category_type, category_id, store_id)
        try:
            ...
            # 写到dataframe里面
            df = pd.DataFrame({'shop_id': shop_id_list, 'original_id': original_id_list, 'name': name_list, 'category': category_list, 'brand_id': brand_id_list, 'shop_type': shop_type_list, 'category_id': category_id_list})
